...
One Anilist entry can run across several TVDB seasons - a long serie that TVDB cuts into four - and only the first of them is the entry's own `tvdb.season`; the rest are in its `season_mapping_list`. Those are read for any season whose own entries hold no first episode, which is the season the serie really runs through with only a movie or a special filed on top of it. A season that already resolves is left exactly as it was.
